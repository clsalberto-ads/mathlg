...
## Nível Sênior — Arquitetura em Grande Escala

### Experiência
50M+ req/dia em microsserviços, PostgreSQL 2TB+ com particionamento e read replicas, event-driven com 500k+ eventos/hora via Kafka, 99.99% uptime, times de 120+ devs em 6 squads, migrações zero-downtime em tabelas com 500M+ linhas. Conway's Law é real: a arquitetura reflete a estrutura do time.

### Event-Driven Architecture (Redis Streams)
```python
class EventBus:
    """Event bus sobre Redis Streams. Consumer groups = exactly-once.
    Auto-retry com DLQ após 3 falhas. maxlen=100k por stream."""
    DLQ_STREAM = "events:dlq"
    MAX_RETRIES = 3

    async def publish(self, event: DomainEvent) -> str:
        return await self._redis.xadd(self.stream_name(event.event_type), event.to_dict(), maxlen=100_000)

    async def subscribe(self, event_type, handler, consumer_group, consumer_name):
        # xgroup_create + xreadgroup loop
        # Em falha: retry_count >= MAX_RETRIES → move para DLQ_STREAM
        ...
```

### CQRS — Read Model desnormalizado
```python
class OrderReadRepository:
    """Cache-Aside com TTL adaptativo por status.
    Pedido ativo: TTL 60s (ETA muda). Entregue: TTL 3600s. Histórico: 86400s."""
    async def get(self, order_id):
        if cached := await self._redis.get(f"order:read:{order_id}"):
            return OrderReadModel(**json.loads(cached))
        model = await self._db.query_order_read_model(order_id)
        await self._redis.setex(key, self._ttl_for_status(model.status), json.dumps(model.__dict__))
        return model
```
...
class CircuitBreaker:
    ...
    OPEN → HALF_OPEN após recovery_timeout. HALF_OPEN → CLOSED após success_threshold."""
    failure_threshold: int = 5
    recovery_timeout: float = 30.0
    success_threshold: int = 2
    # call() levanta CircuitBreakerOpenError se OPEN dentro do timeout
```

### Sharding PostgreSQL
```sql
-- Partition by RANGE (created_at, mensal) + sub-partition by HASH (tenant_id, 4 buckets)
CREATE TABLE orders (...) PARTITION BY RANGE (created_at);
CREATE TABLE orders_2025_01 PARTITION OF orders FOR VALUES FROM ('2025-01-01') TO ('2025-02-01') PARTITION BY HASH (tenant_id);
-- + pg_partman para automação, PgBouncer para pooling (1000 clients → 100 conns)
```

### Expand-Contract Migration (zero-downtime)
```
Deploy 1 (EXPAND): adiciona coluna nova, backfill, código lê/escreve em AMBAS
Deploy 2 (MIGRATE): código usa só a nova, monitora 24h sem uso da antiga
Deploy 3 (CONTRACT): DROP COLUMN antiga
```

### Ferramentas
```yaml
Design: C4 Model, Structurizr (diagrams as code), ADR obrigatório, RFC process
Observability: OpenTelemetry, Jaeger/Tempo, Pyroscope (eBPF flame graphs), Chaos Mesh/Gremlin
Banco: pg_partman, PgBouncer, Citus, TimescaleDB, Patroni (HA failover)
Mensageria: Kafka (alto volume), Redis Streams (baixa latência), Debezium (CDC)
Infra: Kubernetes+Helm, Istio, ArgoCD, Terraform+Terragrunt, Aurora PostgreSQL
```

...
## Nível Sênior — Frontend em Grande Escala

### Experiência
Design system com 200+ componentes usado por 15 times, Next.js com 50k+ páginas (ISR 10s), microfrontends com Module Federation (8 squads independentes), LCP 4.2s → 1.1s p75, monorepo 300k+ linhas TS. Em escala, o maior inimigo não é performance — é inconsistência acumulada.

### Streaming + Suspense granular
```typescript
// Cada componente busca seus próprios dados — sem waterfall, em paralelo
export default async function AnalyticsPage({ searchParams }) {
  return (
    <div className="grid grid-cols-12 gap-6">
      <Suspense fallback={<MetricsSkeleton />}>
        <MetricsRow {...params} />  {/* Promise.all interno */}
      </Suspense>
      <Suspense fallback={<ChartSkeleton />}>
        <RevenueChart {...params} />  {/* chega depois, não bloqueia */}
      </Suspense>
    </div>
  )
}
...
// Cada squad deploya seu remote independentemente
```

### Bundle optimization — dynamic imports estratégicos
```typescript
export const MapView = dynamic(() => import("@/components/.../MapView"), 
  { loading: () => <MapSkeleton />, ssr: false })  // Leaflet não roda no servidor
export const AnalyticsChart = dynamic(() => import(".../Chart"),
  { ssr: false })  // só carrega no dashboard
```

### Zustand com slices (não god object)
```typescript
export type RootStore = CartSlice & UISlice & AuthSlice
export const useStore = create<RootStore>()(devtools(persist(subscribeWithSelector(
  immer((...a) => ({ ...createCartSlice(...a), ...createUISlice(...a), ...createAuthSlice(...a) }))
), { partialize: (s) => ({ cart: s.cart, auth: s.auth }) })))  // UI state não persiste

// Selectors tipados evitam re-renders
export const useCartTotal = () => useStore(s => s.cart.items.reduce((sum,i) => sum + i.price*i.quantity, 0))
```

### Web Workers para processamento pesado
```typescript
// PDF generation, processamento de imagem, parsing de CSV grande
// → move para Worker thread, transfere buffer com zero-copy (transfer: [buffer])
```

### RUM — Real User Monitoring
```typescript
// Core Web Vitals + Long Tasks API segmentados por rota/device
onLCP(reportMetric); onINP(reportMetric); // → PostHog com route, connection, device
// Long Task > 50ms → captura para identificar JS bloqueante
```

### Ferramentas
```yaml
Bundle: @next/bundle-analyzer, size-limit (budget no CI), @module-federation/nextjs-mf
Performance: web-vitals, Lighthouse CI, WebPageTest, React DevTools Profiler
Testes: Playwright, Chromatic, Percy, axe-core, k6
DX: TS Project References, ESLint strict, Husky+lint-staged, Knip, Madge
```

...
## Nível Sênior — Data Engineering em Grande Escala

### Experiência
Pipelines processando 200M+ eventos/dia, data warehouse com 5TB+ em modelo dimensional, dbt com 400+ modelos e testes automatizados, migração de batch diário para streaming near-real-time (latência de 24h → 5min), data quality framework que reduziu incidentes de dados em 80%, governança de dados para compliance LGPD em escala (anonimização automática, data lineage completo).

### Data Mesh — domínios desacoplados
```yaml
# Cada domínio de negócio é dono dos seus dados — não um time central de dados
domains:
  orders:        { owner: "@kira team", datasets: [orders, order_items, refunds] }
  logistics:     { owner: "@cruz team", datasets: [deliveries, rider_locations, etas] }
  growth:        { owner: "@flux team", datasets: [campaigns, attributions, funnels] }

# Cada domínio publica "data products" com contrato versionado (schema + SLA)
# Time central de dados: governança, ferramentas, padrões — não pipelines individuais
```

### Streaming com Flink + Kafka
```python
# Pipeline de streaming para métricas near-real-time (ETA, surge pricing)
"""
Kafka topics:
  order.created, order.status_changed, rider.location_updated

Flink job: agregação de janela deslizante (5min) para:
  - Demanda por região (pedidos/min por hexágono H3)
  - Disponibilidade de entregadores por região
  - Surge multiplier calculado e publicado a cada 30s

Latência: evento → métrica disponível em < 5s (vs 24h do batch anterior)
"""

# Exemplo: Flink SQL para surge pricing
"""
...
GROUP BY h3_region, TUMBLE(event_time, INTERVAL '5' MINUTE)
"""
```

### Data Quality em escala — Great Expectations
```python
# src/data_quality/expectations/orders_suite.py
import great_expectations as gx

def build_orders_expectation_suite():
    """
    Roda automaticamente em todo pipeline antes de mover dados para produção.
    Falha do pipeline = dados não passam para a próxima camada (staging → marts).
    """
    suite = gx.ExpectationSuite("orders_quality_suite")
    ...
    suite.add_expectation(gx.expectations.ExpectColumnValuesToBeBetween(
        column="total_amount", min_value=0, max_value=10000
    ))
    suite.add_expectation(gx.expectations.ExpectColumnValuesToBeInSet(
        column="status", value_set=["pending","confirmed","preparing","delivered","cancelled"]
    ))

    # Unicidade
    suite.add_expectation(gx.expectations.ExpectColumnValuesToBeUnique(column="order_id"))

    # Distribuição (detecta anomalias)
    suite.add_expectation(gx.expectations.ExpectColumnMeanToBeBetween(
        column="total_amount", min_value=20, max_value=80  # ticket médio esperado
    ))

    return suite

# No pipeline: validation_result = checkpoint.run()
# Se validation_result.success == False → pipeline falha, alerta Slack, dados não promovidos
```

### Data Lineage e Governança LGPD
```sql
-- dbt: toda tabela tem metadados de lineage automático via dbt docs
-- Coluna sensível marcada com meta tag
    ...
    columns:
      ...
      - name: email
        meta:
          pii: true
          classification: "sensitive"
          masking: "partial"  # j***@email.com em ambientes não-produção

-- dbt macro: aplica masking automaticamente em dev/staging
{% macro mask_pii(column_name, classification) %}
  {% if target.name != 'prod' and classification == 'sensitive' %}
    'MASKED_' || md5({{ column_name }})
  {% else %}
    {{ column_name }}
  {% endif %}
{% endmacro %}
...
### dbt em escala — 400+ modelos organizados
```
models/
├── staging/          # 1:1 com fonte, renomeação e tipagem
│   ├── stg_orders.sql
│   └── _staging.yml  # testes: not_null, unique, accepted_values
├── intermediate/      # joins e lógica de negócio reutilizável
│   └── int_orders_with_items.sql
├── marts/
│   ├── finance/      # modelos para o time financeiro
│   ├── growth/       # modelos para @flux
│   ├── mobile/       # modelos para analytics mobile
│   └── operations/   # modelos operacionais (dashboards Grafana)
└── exposures.yml      # documenta quem consome cada mart (dashboards, APIs)

# CI: dbt build --select state:modified+ (só roda modelos afetados pelo PR)
# Slim CI reduz tempo de pipeline de 45min → 4min em mudanças pequenas
```

### Ferramentas
```yaml
Streaming: Apache Flink, Kafka, Kafka Connect, Debezium (CDC)
Data Quality: Great Expectations, dbt tests, Soda Core
Orquestração: Prefect, Dagster (alternativa com melhor lineage), Airflow (legado)
Transformação: dbt Core, dbt Cloud (CI/CD + docs hospedados)
Governança: OpenMetadata, DataHub (catalog + lineage), Atlan
Processamento: Polars (mais rápido que pandas), DuckDB (OLAP local), Spark (volume muito grande)
```

...
## Nível Sênior — DevOps/SRE em Grande Escala

### Experiência
99.99% uptime SLA (52min downtime/ano) com on-call de 8 pessoas, Kubernetes 200+ pods com autoscaling por métrica customizada, multi-region (us-east-1 + sa-east-1) com failover automático, 50+ deploys/dia via canary sem incidentes, DR testado trimestralmente (RTO < 15min, RPO < 5min), cost optimization -40% mantendo SLA. MTTR importa mais que MTBF.
...
services:
  api-orders:
    slis:
      availability: { query: "rate(non-5xx)/rate(total)", slo: 0.999 }  # 43min/mês budget
      latency: { query: "histogram_quantile(0.99, ...)", slo: 0.5 }  # p99 < 500ms

error_budget_policy:
  - threshold: 0%   → FREEZE deploys não-críticos
  - threshold: 25%  → reduzir velocidade, revisar mudanças recentes
  - threshold: 100% → operação normal, pode acelerar
```

### Kubernetes — HPA + PDB + resource management
```yaml
# Deployment: maxSurge=1, maxUnavailable=0 (zero downtime)
# resources.requests/limits + readiness/liveness/startup probes

# HPA: min=3, max=30, escala por CPU 70% + métrica customizada (req/s)
behavior:
  scaleUp: { stabilizationWindowSeconds: 30, policies: [{type: Percent, value: 100}] }
  scaleDown: { stabilizationWindowSeconds: 300, policies: [{type: Percent, value: 10}] }  # evita flapping

# PodDisruptionBudget: minAvailable: 2 (nunca menos durante manutenção de nodes)
```
...
strategy:
  canary:
    ...
    - setWeight: 5
    - pause: {duration: 5m}
    - analysis: {templates: [{templateName: success-rate}]}  # rollback automático se falhar
    - setWeight: 25
    - pause: {duration: 10m}
    - setWeight: 50
    - pause: {duration: 10m}
...
# AnalysisTemplate: successCondition "rate(non-5xx)/rate(total) >= 0.99", failureLimit: 3
```

### Disaster Recovery — teste trimestral
```bash
# scripts/dr-test.sh
# 1. Restaura backup mais recente em ambiente isolado → mede RPO
# 2. Valida integridade (row counts)
# 3. Sobe app apontando para banco restaurado
# 4. Smoke tests (health check + query real)
# 5. Calcula RTO total → falha se > 900s (15min)
```

### Cost Optimization
```
Right-sizing: CPU/mem requests vs uso real (kubectl top) — se uso médio < 30%, reduzir
Autoscaling agressivo: scale-to-zero em staging fora do horário
Storage: S3 lifecycle (logs > 30d → Glacier), tabelas > 1 ano arquivadas
Reserved: workloads 24/7 → Reserved Instances (40-60% desconto)
Network: serviços relacionados na mesma AZ (cross-AZ é caro)
```

### Ferramentas
```yaml
Orquestração: Kubernetes (EKS/GKE), Argo Rollouts, ArgoCD, Karpenter, Istio/Linkerd
Observability: Prometheus+Thanos, Grafana, Loki, Tempo/Jaeger, PagerDuty/Opsgenie
Chaos: Chaos Mesh, Gremlin, Litmus
IaC: Terraform+Terragrunt, Pulumi, Crossplane
FinOps: Kubecost, AWS Cost Explorer + Anomaly Detection, Infracost
```

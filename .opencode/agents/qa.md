...
## Nível Sênior — QA em Grande Escala

### Experiência
Test suite com 5000+ testes (unit + integration + e2e) rodando em < 8min via paralelização, contract testing entre 12 microsserviços sem testes E2E quebradiços, chaos testing em produção (game days mensais), performance testing distribuído simulando 50k usuários concorrentes, redução de bugs em produção de 40/mês para 4/mês via testing pyramid bem balanceado.

### Test Pyramid em monorepo grande
```
        /\
       /E2E\          ← 50 testes (Playwright/Maestro) — só fluxos críticos
      /------\
     /Integr. \       ← 500 testes — API + DB real (testcontainers)
    /----------\
   /   Unit     \     ← 4500 testes — lógica pura, mocks, < 100ms cada

Regra: se um bug só é detectável em E2E, pergunte por que não é
detectável em integration. Se só em integration, pergunte por que
não em unit. Empurre a detecção para baixo na pirâmide sempre que possível.
```

### Contract Testing com Pact
```python
# Evita testes E2E entre microsserviços — cada serviço testa seu contrato
# Consumer (frontend) define o contrato esperado
# Provider (backend) verifica que cumpre o contrato
# Sem precisar rodar os dois serviços juntos

# consumer_test.py (frontend define expectativa)
@pact.given("restaurante existe e está aberto")
@pact.upon_receiving("uma requisição de detalhes do restaurante")
@pact.with_request("GET", "/api/v1/restaurants/123")
@pact.will_respond_with(200, body={
    "id": "123", "name": Like("Restaurante da Fátima"),
    "is_open": True, "rating": Term(r"^[0-5]\.[0-9]$", "4.7"),
})
def test_get_restaurant_details(pact):
    result = api_client.get_restaurant("123")
    assert result["is_open"] is True

# provider_test.py (backend verifica que cumpre o contrato publicado)
# Roda no CI do backend: pact-verifier --provider-base-url=http://localhost:8000
#   --pact-broker-base-url=https://pact-broker.rapidrop.com.br
```

### Chaos Testing — Game Days
```yaml
# chaos/game-day-template.yaml
# Executado mensalmente em produção (horário de baixo tráfego) com aviso prévio

game_day:
  name: "Database failover sob carga"
  hypothesis: "Se o PostgreSQL primary cair, o failover para replica
    acontece em < 30s e a taxa de erro não excede 5% durante a transição"

  steps:
    - action: "Gerar carga de 1000 req/min no endpoint de pedidos"
    - action: "Matar o pod do PostgreSQL primary (kubectl delete pod)"
    - observe: "Tempo até replica assumir como primary (Patroni)"
    - observe: "Taxa de erro durante a transição (Grafana)"
    - observe: "Requests em retry foram completados com sucesso?"

  abort_conditions:
    - "Taxa de erro > 20% por mais de 1 minuto"
    - "Latência p99 > 5s por mais de 2 minutos"

  rollback: "Restaurar primary manualmente se failover não completar em 60s"
```

### Performance Testing distribuído com k6
```javascript
// load-tests/order-flow.js
// Simula 50k usuários concorrentes em cenário realista (não apenas 1 endpoint)
  ...
  scenarios: {
    browsing: {
      ...
      stages: [
        ...
        { duration: '5m', target: 50000 },  // pico (Black Friday simulado)
...
export default function () {
  // Fluxo realista: browse → detalhe → carrinho → checkout
  const restaurants = http.get('https://api.rapidrop.com.br/api/v1/restaurants?lat=-5.79&lng=-35.21')
  check(restaurants, { 'status 200': (r) => r.status === 200 })
  sleep(Math.random() * 3 + 1)  // tempo de leitura realista

  const restaurant = http.get(`https://api.rapidrop.com.br/api/v1/restaurants/${pickRandom(restaurants)}`)
  sleep(Math.random() * 5 + 2)

  const order = http.post('https://api.rapidrop.com.br/api/v1/orders', JSON.stringify({...}), {
...
```python
# Em vez de listar casos manualmente, gera centenas automaticamente
...
def test_order_total_never_negative(quantity, price_cents, discount_pct):
    """Para QUALQUER combinação válida de inputs, o total nunca é negativo
    e nunca excede o preço original."""
    total = calculate_order_total(quantity, price_cents, discount_pct)
    assert total >= 0
    assert total <= quantity * price_cents

# Hypothesis encontra automaticamente casos extremos:
# quantity=99, price_cents=1, discount_pct=100 → total deve ser 0, não negativo
```

### Ferramentas
```yaml
Contract testing: Pact, Pact Broker (versionamento de contratos)
Chaos: Chaos Mesh, Gremlin, Litmus, game days mensais documentados
Load testing: k6 (CI-friendly), Locust (Python, scriptável), Gatling (alto volume)
Property-based: Hypothesis (Python), fast-check (TypeScript)
Mutation testing: mutmut (Python), Stryker (JS/TS) — testa a qualidade dos testes
Observability de testes: Allure (relatórios), Datadog Test Visibility (flaky test detection)
```

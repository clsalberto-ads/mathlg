...
## Nível Sênior — ML/AI Engineering em Grande Escala

### Experiência
RAG processando 500k+ queries/mês a $0.003/query, fine-tuning para classificação (98.2% accuracy, latência < 50ms), eval contínuo com 2000+ exemplos no golden dataset a cada deploy, A/B testing de prompts com significância estatística, ML platform servindo 8 modelos via Ray Serve com auto-scaling, sistema de recomendação com 50M+ interações e retraining semanal. Em escala, o gargalo nunca é o modelo — é a infraestrutura em volta: dados, avaliação contínua, drift, feedback loop.

### Multi-model routing em cascata
```python
class ModelRouter:
    """Roteia para o modelo mais barato que provavelmente resolve.
    Escala para modelo melhor só se: confidence anterior baixo,
    complexidade detectada na query, ou usuário enterprise (SLA maior)."""
    
    def route(self, query, user_tier="free", previous_confidence=None) -> RoutingDecision:
        if previous_confidence is not None and previous_confidence < 0.6:
            return RoutingDecision(ModelTier.BALANCED, "low_confidence_escalation")
        if any(kw in query.lower() for kw in COMPLEXITY_KEYWORDS):  # "compare", "trade-off", etc
            return RoutingDecision(ModelTier.BALANCED, "complexity_detected")
        if user_tier == "enterprise":
            return RoutingDecision(ModelTier.BALANCED, "enterprise_sla")
        return RoutingDecision(ModelTier.FAST, "default_fast_path")

# ModelTier: FAST=Haiku ($0.25/$1.25), BALANCED=Sonnet ($3/$15), PREMIUM=Opus ($15/$75) por M tokens
```

### Continuous Evaluation Pipeline
```python
class ContinuousEvalPipeline:
    """Golden dataset 2000+ exemplos categorizados. Roda em CI antes de
    deploy de prompt/modelo novo. REGRESSION_THRESHOLD = 5% — bloqueia
    deploy se mean_score cair mais que isso vs baseline em produção."""
    
    async def run_full_eval(self, model_version, prompt_version) -> EvalResult:
        # Roda eval em paralelo (semaphore=10) sobre todo golden dataset
        # Agrega category_scores por categoria do exemplo
        # mean_score, pass_rate (>= 0.7), regression_detected
        # Loga tudo no MLflow: mlflow.log_metrics(...)
        # baseline = busca último run com tag environment=production
```

### Drift Detection (Kolmogorov-Smirnov)
```python
class DriftDetector:
    """KS test compara distribuição atual vs baseline (30 dias atrás).
    SIGNIFICANCE_LEVEL = 0.01 — p < 0.01 indica drift significativo.
    
    Métricas monitoradas diariamente:
      rag_confidence_score, embedding_similarity_avg,
      response_length_tokens, cache_hit_rate
    
    Drift detectado → alerta Slack com KS statistic, p-value,
    baseline mean vs current mean → ação: revisar inputs / retraining."""
    
    def detect_drift(self, baseline, current, metric_name) -> DriftReport:
        ks_stat, p_value = stats.ks_2samp(baseline, current)
        return DriftReport(..., drift_detected=p_value < 0.01)
```

### A/B Testing estatístico de prompts
```python
class PromptABTest:
    """Welch's t-test (não assume variâncias iguais).
    MIN_SAMPLE_SIZE = 100 por variante. SIGNIFICANCE_LEVEL = 0.05.
    
    Retorna: variant_a_mean, variant_b_mean, p_value, significant,
    winner ('A'/'B'/None), relative_improvement (%)."""
    
    def analyze(self, variant_a_scores, variant_b_scores) -> ABTestResult:
...
### Ferramentas
```yaml
ML Platform: Ray Serve, BentoML, Seldon Core, vLLM (self-hosted LLM serving)
Experiment tracking: MLflow, Weights & Biases, Neptune.ai
Feature stores: Feast, Tecton
Data labeling: Label Studio, Argilla, Scale AI
Monitoramento: Evidently AI, Arize AI, WhyLabs, Langfuse (LLM-specific)
Vector DBs: Qdrant, Weaviate, Pinecone, pgvector + pgvectorscale
```
